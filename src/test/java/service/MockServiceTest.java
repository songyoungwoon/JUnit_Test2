...
public class MockServiceTest {
    ...
    @Test
    public void shouldAtLeastTwoTimesRunForChampionGetName(){
        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
    ...
    @Test
    public void shouldAtMostThreeTimesRunForChampionGetName(){
        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
    ...
    @Test
    public void shouldPassWhenNeverRunForChampionSetName(){
        Champion topChamp = mock(Champion.class);
        //topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
    ...
    @Test
    public void shouldTwoHundredTimeoutAtLeastOneRunForChampionGetName(){
        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
    ...
    @Test
    public void 챔피언리스트개수확인테스트(){
        List<Champion> mockChampions = new ArrayList<>();
        mockChampions.add(new Champion("리산드라", "미드", 4));
        mockChampions.add(new Champion("다리우스", "탑", 3));
        mockChampions.add(new Champion("킨드레드", "정글", 4));
    ...
    @Test
    public void 챔피언검색시가짜챔피언리턴레포지토리한번호출검증스킨개수확인(){
        when(mockService.findByName(anyString())).thenReturn(new Champion("아리", "미드", 5));
        assertThat(mockRepository.findByName("람머스").getHasSkinCount(), is(5));
    ...
    @Test
    public void 가짜챔피언리스트테스트 (){
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("아리", "미드", 5));
        champions.add(new Champion("제드", "미드", 4));
        champions.add(new Champion("르블랑", "미드", 3));
        when(mockService.findAllChampions()).thenReturn(champions);
        assertThat(mockService.findAllChampions().get(0).getName(), is("아리"));
        assertThat(mockService.findAllChampions().get(1).getHasSkinCount(), is(4));
